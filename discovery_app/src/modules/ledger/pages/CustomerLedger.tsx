...
import { selectAllLedger } from "../features/ledgerSelectors.ts";
...
export default function CustomerLedger() {
  ...
  const handleEdit = (ledger: Ledger) => {
    ...
    if(ledger.transactionType == "payment_in" || ledger.transactionType == "payment_out"){
      console.log(ledger.referenceId);
  ...
  const handleDelete = async (ledger: Ledger) => {
    ...
    if (["purchase", "sale", "return"].includes(ledger.transactionType)) {
      await dispatch(destroy(ledger.referenceId));
    }
          ...
          <TabPanels>
            ...
            <TabPanel className="space-y-6">
              <div className="p-4 mb-4 border border-gray-300 rounded-lg bg-white dark:bg-gray-800 space-y-4">
                ...
                <CurrencyInvoice editingLedgerId={editingLedgerId ?? 0}/>
              </div>
            </TabPanel>
            ...
            <TabPanel>
              <div className="p-4 mb-4 border border-gray-300 rounded-lg bg-white dark:bg-gray-800 space-y-4">
                ...
                <CurrencyPayment editingPaymentId={editingPaymentId ?? 0}/>
              </div>
            </TabPanel>
          </TabPanels>
        </TabGroup>
            ...
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
               {status === 'loading' ? (
                  ...
                  paginatedLedgers.map((ledger, index) => (
                    <TableRow key={ledger.id} className="border-b border-gray-100 dark:border-white/[0.05]">
                      ...
                      <TableCell className="text-center px-4 py-2 text-sm overflow-visible">
                       ...
                       <Button size="sm" className="ml-1" type="button" variant="outline" onClick={() => handleDelete(ledger)}>
                ...
                <TableRow>
                  <TableCell colSpan={6} isHeader className="border border-gray-500 text-center px-4 py-2">Total:</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{purchaseDebit.toFixed(2)}</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{purchaseCredit.toFixed(2)}</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{saleDebit.toFixed(2)}</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{saleCredit.toFixed(2)}</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{' '}</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell colSpan={6} isHeader className="border border-gray-500 text-center px-4 py-2">Balance:</TableCell>
                  <TableCell colSpan={2} isHeader className="border border-gray-500 text-center px-4 py-2">{purchaseBalance.toFixed(2)}</TableCell>
                  <TableCell colSpan={2} isHeader className="border border-gray-500 text-center px-4 py-2">{saleBalance.toFixed(2)}</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{' '}</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell colSpan={6} isHeader className="border border-gray-500 text-center px-4 py-2">Close Balance:</TableCell>
                  <TableCell colSpan={4} isHeader className="border border-gray-500 text-center px-4 py-2">{closeBalance.toFixed(2)}</TableCell>
                  <TableCell isHeader className="border border-gray-500 text-center px-4 py-2">{' '}</TableCell>
                </TableRow>
              </TableBody>
